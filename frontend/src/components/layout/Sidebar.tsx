...
function ModeLinks({ onNavigate }: { onNavigate?: () => void }) {
...
export function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <ScrollArea className="h-full w-full p-3">
      ...
      >
        <ModeLinks onNavigate={onNavigate} />
      </Suspense>
      ...
      <div className="space-y-0.5">
        {isLoading ? (
          ...
          sessions.map((session, index) => {
            const displayDate = new Date(session.created_at).toLocaleDateString(
              undefined,
              {
                month: "short",
                day: "numeric",
            ...
            return (
              ...
              >
                <div className="flex items-center gap-1.5 w-full">
                  <LineChart className="h-3 w-3 text-chart-1 shrink-0" />
                  <span
                    title={session.problem_title || "Unknown Problem"}
                    className="font-medium text-xs truncate text-foreground group-hover:text-primary transition-colors"
                  >
...
export function Sidebar() {
  return (
    <aside className="fixed top-11 left-0 bottom-0 w-54 border-r border-border/40 bg-white/50 backdrop-blur-sm hidden lg:block z-40">
      <SidebarContent />
    </aside>
  );
}
