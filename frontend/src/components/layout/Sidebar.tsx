  ...
  return (
    <div className="space-y-0.5 mb-4">
      ...
      >
        <Lightbulb
          className={`h-3.5 w-3.5 transition-colors ${
            mode === "analysis" ? "text-blue-600" : "text-slate-400"
          }`}
        />
        Problem Analysis
      </Link>
      ...
      >
        <FileCode2
          className={`h-3.5 w-3.5 transition-colors ${
            mode === "review" ? "text-blue-600" : "text-slate-400"
          }`}
        />
        Code Review
      </Link>
...
export function Sidebar() {
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case "Analysis":
        return <LineChart className="h-3 w-3 text-purple-500" />;
      case "Interview":
  ...
  return (
    <aside className="fixed top-11 left-0 bottom-0 w-48 border-r border-border/40 bg-white/50 backdrop-blur-sm hidden lg:block p-3 overflow-y-auto z-40">
      ...
      <Suspense
        fallback={
          <div className="h-16 bg-slate-100/50 rounded-md animate-pulse mb-4 mx-2" />
        }
      >
        <ModeLinks />
      </Suspense>
      ...
      <div className="space-y-0.5">
        {MOCK_RECENT_SESSIONS.map((session, index) => (
          <button
            type="button"
            key={session.id}
            className={`w-full flex flex-col items-start gap-0 px-2.5 py-1.5 rounded-md hover:bg-slate-100/60 transition-all group text-left animate-slide-up stagger-${
              (index % 5) + 1
            }`}
          >
            <div className="flex items-center gap-1.5 w-full">
              {getIcon(session.type)}
              <span className="font-medium text-xs truncate text-slate-700 group-hover:text-blue-600 transition-colors">
                {session.title}
              </span>
            </div>
